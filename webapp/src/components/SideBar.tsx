...
const SideBar = () => {
    ...
    return (
        <>
            ...
            <aside className='flex flex-col justify-between fixed inset-0 bg-white w-16 xl:w-64 h-full border-r border-grey-500 pt-4 dark:bg-black dark:border-gray-800 hidden sm:flex'>
                {showSignOutModal &&
                    <Modal changeModal={changeSignOutModal}>
                        <div className="flex flex-col justify-center p-4 text-center">
                            <h2 className="text-xl font-semibold mb-2 dark:text-white">Confirm Sign Out</h2>
                    ...
                    <ul className="space-y-3 font-medium px-3 mt-4">
                        <li>
                            <Link onClick={() => changeCurrentRoute(ConstRoutes.HOME)} to={ConstRoutes.HOME} className={`flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-900 group dark:text-white transition-all duration-300 ${current === ConstRoutes.HOME ? 'bg-gray-100 dark:bg-gray-900' : ''}`}>
                                ...
                                <span className="hidden xl:block ml-3">Home</span>
                            </Link>
                        </li>
                        <li>
                            <Link onClick={() => changeCurrentRoute(ConstRoutes.MY_PROFILE)} to={ConstRoutes.MY_PROFILE} className={`flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-900 group dark:text-white transition-all duration-300 ${current === ConstRoutes.MY_PROFILE ? 'bg-gray-100 dark:bg-gray-900' : ''}`}>
                                ...
                                />
                                <span className="hidden xl:block ml-3">Profile</span>
                            </Link>
                        </li>
                        <li className='pt-8'>
                            <button onClick={changeShowNewPostModal} className="flex items-center justify-center bg-yellow-500 p-2 rounded-lg font-semibold text-white w-full transition duration-300 ease-in-out hover:bg-yellow-600">
                                <img src={plusIcon} alt="Ícone" className="w-5 h-5" />
                                <span className='ml-2 hidden xl:block'>New Post</span>
                            </button>
                        </li>
                    </ul>
                </div>
                <div className='px-2 pb-4'>
                    <div onClick={changeThemeMode} className='flex items-center cursor-pointer mb-3 p-2'>
                        <img className='h-6 w-6' src={window.localStorage.getItem('theme')?.includes('dark') ? sunIcon : moonIcon} alt="light toggle" />
                        <span className='hidden xl:block font-bold text-yellow-500 ml-2'>{window.localStorage.getItem('theme')?.includes('dark') ? 'Light mode' : 'Dark mode'}</span>
                    </div>
                    <div onClick={changeSignOutModal} className="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-900 cursor-pointer dark:text-white transition-all duration-300">
                        ...
                        <span className="hidden xl:block ml-3">Logout</span>
                    </div>
                </div>
            </aside>
        </>
